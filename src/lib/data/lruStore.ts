...
export function lru<S extends zarr.Readable>(store: S, maxSize = 300) {
  const cache = new QuickLRU<string, Promise<Uint8Array | undefined>>({
    maxSize,
  });
  ...
  if (getRange) {
    ...
    getRange = (...args: Parameters<NonNullable<S["getRange"]>>) => {
      ...
      const result = Promise.resolve(_getRange?.(key, range, opts)).catch(
        (err) => {
          cache.delete(cacheKey);
          throw err;
        }
